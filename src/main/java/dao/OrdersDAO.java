...
public class OrdersDAO {
    ...
    public void insertOrder(List<Product> list, Account acc, String name, String address, String upDate) throws Exception {

        try {
            for (int i = 0; i < list.size(); i++) {
                String query = "insert into bookstoredb.orders(user_mail, user_name, user_phone, user_address, " +
                        "product_id, product_name, product_quantity, product_price, order_date)\n" +
                        "values(?,?,?,?,?,?,?,?,?)";
